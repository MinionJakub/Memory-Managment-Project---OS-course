...
static void split(word_t *placement, size_t size, size_t size_of_placement) {
  ...
  if (prev == -1) {
    ...
    set_val_next(previous, place - previous);
    set_val_prev(place, place - previous);
  }
  if (next == -1)
    set_val_next(place, -1);
  else {
    word_t *nextling = placement + next;
    set_val_prev(nextling, nextling - place);
    set_val_next(nextling, nextling - place);
  }
}
...
void free(void *ptr) {
  ...
  if (next == 0) {
    ...
    if (prev_next == NULL)
      ...
      next = prev_next - header;
  }
  if (prev == 0) {
    if (prev_next == NULL)
      prev = -1;
    else
      next = header - prev_next;
  }
  if (!prev_free)
    free_list = header;
